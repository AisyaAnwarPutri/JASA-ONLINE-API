...
exports.findUsers = function(req, res) {
    ...
    connection.query('SELECT * FROM tbl_user where id = ?', [ user_id ], function (error, rows, fields){
        if(error){
...
exports.registerUser = async function(req, res) {
    ...
    if (!nama || !tanggal_lahir || !jenis_kelamin || !nomor_hp || !alamat || !email || !plain_password) {
        message = "Kehilangan beberapa parameter yang dibutuhkan!";
        return response.gagal(message, res);
    }
    
    await connection.query('SELECT * FROM tbl_user where email = ?', [ email ], function(error, rows, fields){
        if(rows.length > 0){
            message = "Email user telah terdaftar, silahkan pilih yang lain!";
            response.gagal(message, res);
        }else{
            var encrypt_password = encrypt.saltHashPassword(plain_password);
            connection.query('INSERT INTO tbl_user (nama, tanggal_lahir, jenis_kelamin, nomor_hp, alamat, email, password) values (?,?,?,?,?,?,?)', [ nama, tanggal_lahir, jenis_kelamin, nomor_hp, alamat, email, encrypt_password ], function (error, rows, fields){
                if(error){
...
exports.updateUser = async function(req, res) {
    ...
    var params;
    if(plain_password == ""){
        sql_query = 'UPDATE tbl_user SET nama = ?, tanggal_lahir = ?, jenis_kelamin = ?, nomor_hp = ?, alamat = ?, email = ? WHERE id = ?';
        params = [ nama, tanggal_lahir, jenis_kelamin, nomor_hp, alamat, email, user_id ]
    }else{
        sql_query = 'UPDATE tbl_user SET nama = ?, tanggal_lahir = ?, jenis_kelamin = ?, nomor_hp = ?, alamat = ?, email = ?, password = ? WHERE id = ?';
        var encrypt_password = encrypt.saltHashPassword(plain_password);
        params = [ nama, tanggal_lahir, jenis_kelamin, nomor_hp, alamat, email, encrypt_password, user_id ]
    }
    
    await connection.query('SELECT * FROM tbl_user where email = ? and id <> ?', [ email, user_id ], function(error, rows, fields){
        if(rows.length > 0){
            message = "Email user telah terdaftar, silahkan pilih yang lain!";
            response.gagal(message, res);
        }else{
...
exports.deleteUser = async function(req, res) {
    ...
    await connection.query('SELECT * FROM tbl_user where id = ?', [ user_id ], function(error, rows, fields){
        if(rows.length > 0){
            connection.query('DELETE FROM tbl_user WHERE id = ?', [ user_id ], function (error, rows, fields){
                if(error){
...
//for request with image multipart
var helpers = require('./helpers');
var multer = require('multer');
var fs = require('fs');

exports.addService = function(req, res){
    let upload = multer({storage: helpers.storage, limits: { fileSize: MAX_SIZE }, fileFilter: helpers.imageFilter}).single('file');
    upload(req, res, function(err) {
        ...
        if (!id_user || !nama_jasa || !deskripsi_singkat || !uraian_deskripsi || !rating || !gambar) {
            message = "Kehilangan beberapa parameter yang dibutuhkan!";
            return response.gagal(message, res);
        }
        ...
        connection.query('INSERT INTO tbl_jasa (id_user, nama_jasa, deskripsi_singkat, uraian_deskripsi, rating, gambar) values (?,?,?,?,?,?)', [ id_user, nama_jasa, deskripsi_singkat, uraian_deskripsi, rating, gambar], function (error, rows, fields){
            if(error){
                //removeImage;
...
exports.getServices = function(req, res){
    connection.query('SELECT tbl_jasa.id_jasa, tbl_user.nama as nama_penyedia, tbl_user.nomor_hp, tbl_jasa.nama_jasa, tbl_jasa.deskripsi_singkat, tbl_jasa.uraian_deskripsi, tbl_jasa.rating, tbl_jasa.gambar FROM tbl_jasa, tbl_user WHERE tbl_user.id = tbl_jasa.id_user ORDER BY tbl_jasa.id_jasa DESC', function (error, rows,fields){
        var message;
        if(error){
...
exports.getUserServices = function(req, res){
    ...
    connection.query('SELECT tbl_jasa.id_jasa, tbl_user.nama as nama_penyedia, tbl_user.nomor_hp, tbl_jasa.nama_jasa, tbl_jasa.deskripsi_singkat, tbl_jasa.uraian_deskripsi, tbl_jasa.rating, tbl_jasa.gambar FROM tbl_jasa, tbl_user WHERE tbl_user.id = tbl_jasa.id_user AND tbl_user.id = ? ORDER BY tbl_jasa.id_jasa DESC', [ user_id ], function (error, rows, fields){
        if(error){
...
exports.deleteService = async function(req, res){
    ...
    await connection.query('SELECT * FROM tbl_jasa where id_jasa = ?', [ id_jasa ], function(error, rows, fields){
        if(rows.length > 0){
            var filename = rows[0].gambar
            connection.query('DELETE FROM tbl_jasa WHERE id_jasa = ?', [ id_jasa ], function (error, rows, fields){
                if(error){
...
exports.editServiceReplaceImage = function(req, res){
    ...
    upload(req, res, async function(err) {
        ...
        if (!id_jasa || !nama_jasa || !deskripsi_singkat || !uraian_deskripsi || !gambar) {
            message = "Kehilangan beberapa parameter yang dibutuhkan!";
            return response.gagal(message, res);
        }
        ...
        await connection.query('SELECT * FROM tbl_jasa where id_jasa = ?', [ id_jasa ], function(error, rows, fields){
            if(rows.length > 0){
                var sql_query = 'UPDATE tbl_jasa SET nama_jasa = ?, deskripsi_singkat = ?, uraian_deskripsi = ?, gambar = ? WHERE id_jasa = ?';
                var params = [ nama_jasa, deskripsi_singkat, uraian_deskripsi, gambar, id_jasa ]
                var filename = rows[0].gambar
                connection.query(sql_query, params, function (error, rows, fields){
                    if(error){
...
exports.editService = async function(req, res){
    ...
    await connection.query('SELECT * FROM tbl_jasa where id_jasa = ?', [ id_jasa ], function(error, rows, fields){
        if(rows.length > 0){
            var sql_query = 'UPDATE tbl_jasa SET nama_jasa = ?, deskripsi_singkat= ?, uraian_deskripsi = ? WHERE id_jasa = ?';
